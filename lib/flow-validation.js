...
export function validateFlow(flow) {
  ...
  const hasCycle = topoOrder.length !== flow.nodes.length;
  // Preserve stable source order for cyclic components; DAGs keep deterministic
  // dependency order so existing CLI output remains unchanged.
  const order = hasCycle
    ? [...topoOrder, ...flow.nodes.map((node) => node.id).filter((id) => !topoOrder.includes(id))]
    : topoOrder;
  ...
  const maxDepth = hasCycle ? 0 : Math.max(0, ...depth.values());
  return {
    flow,
    order,
    cyclic: hasCycle,
    levels: hasCycle ? [order] : Array.from({ length: maxDepth + 1 }, (_, level) => order.filter((id) => depth.get(id) === level))
  };
}
