...
function renderWorkflowStructure(flow, lang = "zh") {
  const order = orderedNodeIds(flow);
  const nodeById = new Map(flow.nodes.map((node) => [node.id, node]));
  ...
  const steps = order.map((nodeId, index) => {
    ...
    const dependency = parents.length
      ? `${en ? "; depends on " : "；依赖："}${parents.join(en ? ", " : "、")}`
      : en ? "; entry point" : "；起点";
  ...
  const logicLines = logic.conditions.length === 0
    ...
    : [
        "",
        en ? "## Logic-gate execution contract" : "## 逻辑门执行契约",
        "",
        en
          ? "> A condition box is not a decorative label: incoming edges provide the operands, the predicate converts upstream results to Boolean values, and the standard truth table decides the outcome."
        ...
        ...logic.conditions.map((condition) => {
          const inputs = condition.inputs
            ...
            .join(en ? ", " : "、") || (en ? "not connected" : "未连接");
          const output = condition.gateType === "ifElse"
            ? en ? "true takes \"Yes\", false takes \"No\"" : "结果为真走“是”，结果为假走“否”"
            : en
              ? "the result propagates through every gate output; truth activates ordinary steps, falsity can still feed downstream gates"
              : "结果沿全部门输出箭头传播；真值激活普通步骤，假值仍可输入下游逻辑门";
          return `- **${condition.label}** · \`${condition.gateType}\` · \`${condition.formula}\`；${en ? "inputs" : "输入"}：${inputs}；${en ? "output" : "输出"}：${output}`;
        }),
  ...
  return [
    STRUCTURE_START,
    en ? "## Workflow info" : "## 工作流信息",
    "",
    `- ID：\`${flow.id}\``,
    en ? `- Schema version: \`${flow.version ?? 1}\`` : `- Schema 版本：\`${flow.version ?? 1}\``,
    en ? `- Execution revision: \`rev ${flow.revision ?? 1}\`` : `- 执行修订：\`rev ${flow.revision ?? 1}\``,
    en ? `- Steps: ${order.length}` : `- 步骤数：${order.length}`,
    "",
    en ? "## Execution order" : "## 执行顺序",
    "",
    ...steps,
    ...logicLines,
    STRUCTURE_END
...
export function renderWorkflowDocument(flow, lang = "zh") {
  const description = String(flow.description ?? "").trim();
  const en = lang === "en";
  return [
    `# ${flow.name ?? flow.id}`,
    "",
    description || (en ? "A document-driven workflow managed by AgentFlow." : "这是由 AgentFlow 管理的文档驱动工作流。"),
    "",
    en
      ? "> Read this file first to understand the execution order, then open the STEP.md inside each step directory. The Flow Markdown editor syncs these files directly."
      : "> 先阅读本文件掌握执行顺序，再进入每个步骤目录阅读对应的 STEP.md。Flow 中的 Markdown 编辑器会直接同步这些文件。",
    "",
    renderWorkflowStructure(flow, lang),
    "",
    en ? "## Execution conventions" : "## 执行约定",
    "",
    ...(en
      ? [
          "1. Follow the dependency order above strictly; parallel branches follow the graph connections.",
          "2. Read the corresponding `STEP.md` before starting each step, and keep artifacts in that step's workspace.",
          "3. Debug steps keep key screenshots, error messages, and fix conclusions.",
          "4. QA steps verify functionality, build, regression, and UI layout before giving the final verdict.",
          "5. At a condition box, take the upstream result, apply the predicate, and compute the truth value per the logic-gate contract; never treat AND/OR/XOR as plain text labels."
        ]
      : [
...
function refreshWorkflowStructure(content, flow, lang = "zh") {
  const start = content.indexOf(STRUCTURE_START);
  const end = content.indexOf(STRUCTURE_END);
  if (start < 0 || end < start) return content;
  return `${content.slice(0, start)}${renderWorkflowStructure(flow, lang)}${content.slice(end + STRUCTURE_END.length)}`;
}
...
export async function writeFlowDocuments(flow, options = {}) {
  if (!flow?.docRoot || !flow?.docs) return flow;
  const root = resolve(flow.docRoot);
  ...
  const workflowContent = (currentContent
    ? flow.workflowStructureSignature === structureSignature
      ? currentContent
      : refreshWorkflowStructure(currentContent, flow, lang)
    : renderWorkflowDocument(flow, lang)).trimEnd();
